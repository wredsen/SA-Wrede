...
Die sich auf die jeweilige Koordinate $\theta_i$ beziehende Stellkraft $Q_i$ entspricht der verallgemeinerten Kraft, welche sich aus äußeren (Stell-)Kräften sowie internen Reibungskräften zusammensetzt \cite[S.7]{DissKnoll}. Vorteile dieser Herangehensweise sind, dass Zwangskräfte nicht explizit bestimmt werden müssen und die Durchführung beispielsweise unter Nutzung des Python-Pakets \textit{symbtools} \cite{symbtools}.
...
Eine alternative Herleitung solcher Systemgleichungen sind die Lagrange-Gleichungen erster Art. \cite{Lagrange1}
