...
Die Dynamik mechanischer Systeme lässt sich über Differentialgleichungen, den Lagrange-Gleichungen zweiter Art, beschreiben. Die Menge aus $n$ auftretenden und zeitlich veränderlichen Koordinaten werden auch als Konfigurationskoordinaten oder Systemgrößen $\pmb{\theta} = (\theta_1, ..., \theta_n)^T$ bezeichnet. Die zeitliche Änderung dieser sind die s (Konfigurations-)Geschwindigkeiten $\dot{\pmb{\theta}}$. Die kinetische Energie eines Systems wird im Folgenden durch die Funktion $T(\pmb{\theta}, \dot{\pmb{\theta}})$ sowie die potentielle Energie durch $V(\pmb{\theta})$ beschrieben.

DissKnoll \cite{DissKnoll} dqwdqwdqw -> dazu in Nextcloud/Präsis Folien zu modeltools, ggf. noch Diss von Knoll
\subsection{Lagrange-Gleichungen erster Art}
Eine alternative Herleitung solcher Systemgleichungen sind die Lagrange-Gleichungen erster Art.  
