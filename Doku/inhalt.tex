...
Problem:\\
- Simulationsergebnis bei ungestörtem Vorsteuerungsfall mit sehr schlechtem Folgeverhalten, wenn auch stationärer Genauigkeit
- divergiert bei bei Anfangsfehlern\\
...
\begin{align}
	\begin{split}
		\ddot{p}_2 &= L_{\pmb{\tilde{\alpha}}} \dot{y}_2 = L_{\pmb{\tilde{\alpha}}} \dot{p}_2 \\
		p_2^{(3)} &= L_{\pmb{\tilde{\alpha}}} \ddot{p}_2.
	\end{split}
\end{align}
...
\begin{equation}
	v_2 = y_2^{(4)} = y_{\text{ref}}^{(4)} - c_{2, 3} (y^{(3)} - y_{\text{ref}}^{(3)}) - c_{2, 2} (\ddot{y} - \ddot{y_{\text{ref}}}) - c_{2, 1} (\dot{y} - \dot{y_{\text{ref}}}) - c_{2, 0} (y - y_{\text{ref}})
\end{equation} 
